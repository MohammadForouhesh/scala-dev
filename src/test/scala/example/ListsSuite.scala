...
class ListsSuite extends munit.FunSuite:
    ...
    test("sum of negative numbers (10pts)") {
        assertEquals(sum(List(-3, -7, -2)), -12)
    }

    test("sum of mixed numbers (10pts)") {
        assertEquals(sum(List(3, -7, -2)), -6)
    }

    test("max of mixed numbers (10pts)") {
        assertEquals(max(List(3, 0, -2)), 3)
    }
