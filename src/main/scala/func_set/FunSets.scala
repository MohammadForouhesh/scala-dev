...
trait FunSets extends FunSetsInterface:
    ...
    def forall(s: FunSet, p: Int => Boolean): Boolean =
        def iter(a: Int): Boolean =
            if a > bound then
                true
            else if !this.filter(s, p)(a) then
                false
            else
                iter(a + 1)
        iter(-bound)
    ...
    /**
      ...
      * that satisfies `p`.
      */
    def exists(s: FunSet, p: Int => Boolean): Boolean = (-bound to bound).map(this.filter(s, p)(_)).foldLeft(false)(_ || _)

    /**
      * Returns a set transformed by applying `f` to each element of `s`.
      */
    def map(s: FunSet, f: Int => Int): FunSet = (x: Int) => this.exists(s, (y: Int) => x == f(y))
